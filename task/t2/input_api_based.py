...
class SearchRequests(BaseModel):
    search_request_parameters: list[SearchRequest] = Field(default=[])


def retrieve_context(user_question: str) -> list[User]:
    ...
    search_requests: SearchRequests = (prompt | client | parser).invoke({})
    ...
    if hasattr(search_requests, "search_request_parameters"):
        requests_dict = {}
...
def augment_prompt(user_question: str, context: list[User]) -> str:
...
def generate_answer(augmented_prompt: str) -> str:
    ...
    response = client.invoke(input=[system_message, human_message])
